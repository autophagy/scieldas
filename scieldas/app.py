...
def rtd(project, filetype):
    return image_creator.create_image(filetype, api.get_rtd_build_status, project), filetype

# Travis
@application.route("/travis/<user>/<project>.<filetype>")
@create_image_response
def travis(user, project, filetype):
    return image_creator.create_image(filetype, api.get_travis_build_status, user, project), filetype
...
def pyversions(project, filetype):
    return image_creator.create_image(filetype, api.get_pypi_pyversions, project), filetype
...
def docker_build(user, project, filetype):
    return image_creator.create_image(filetype, api.get_docker_build_status, user, project), filetype
